...
trait HandleDay
{
    ...
    public function updateTimesheet($checkIn, $checkOut)
    {
        ...
        if (config('constant.eight_AM') - $sumOut > config('constant.positive_integer') || !isset($checkIn) || now()->parse()->isWeekend()) {
    ...
    public function updateTimesheetApp($checkIn, $checkOut)
    {
        $sub = now()->parse($checkOut)->diffInSeconds($checkIn);
        $actualWorkingTime = gmdate("H:i:s", $sub);
        //Convert hours to seconds
